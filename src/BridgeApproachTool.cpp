...
public:
	...
	void CreateBridgeApproaches(int startTileX, int startTileZ, int endTileX, int endTileZ,
		...
		if (bridgeLength == 0) {
			mLogger->WriteLineFormatted(LogLevel::Error, "Bridge start and end are the same point");
			return;
		}
	...
	float CalculateOptimalApproachLength(int bridgeX, int bridgeZ, float dirX, float dirZ,
		...
		for (int iteration = 0; iteration < maxIterations; iteration++) {
			...
			if (std::abs(requiredLength - currentLength) < convergenceThreshold) {
				mLogger->WriteLineFormatted(LogLevel::Info,
					"Converged to approach length: %.1f tiles (height difference: %.2f meters)",
		...
		mLogger->WriteLineFormatted(LogLevel::Error,
			"Approach length calculation didn't converge, using: %.1f tiles", currentLength);
