...
public:
		...
		, mSecondaryTextID(controlID + 101)
	{
		...
		LOG_DEBUG("GenericDragViewInputControl created: {}", toolName.c_str());
	}
	...
	bool OnKeyDown(int32_t vkCode, uint32_t modifiers) override {
		LOG_TRACE("OnKeyDown: keyCode=0x{:X}, modifiers=0x{:X}", vkCode, modifiers);
...
private:
	ParameterType GetScrollTargetParameter(uint32_t modifiers) {
		bool ctrlPressed = (modifiers & Ctrl) != 0;
		bool shiftPressed = (modifiers & Shift) != 0;
		bool altPressed = (modifiers & Alt) != 0;
		
		LOG_TRACE("GetScrollTargetParameter: modifiers=0x{:X}, ctrl={}, shift={}, alt={}",
		...
		if (altPressed) {
			if (ctrlPressed && shiftPressed) {
				LOG_TRACE("Selected parameter: Fourth (Alt+Ctrl+Shift)");
				return ParameterType::Fourth;
			} else if (ctrlPressed) {
				LOG_TRACE("Selected parameter: Third (Alt+Ctrl)");
				return ParameterType::Third;
			} else if (shiftPressed) {
				LOG_TRACE("Selected parameter: Second (Alt+Shift)");
				return ParameterType::Second;
			} else {
				LOG_TRACE("Selected parameter: First (Alt only)");
				return ParameterType::First;
			}
		}
		
		// This should not be reached since we only call this when Alt is pressed
		LOG_INFO("Selected parameter: First (fallback)");
		return ParameterType::First;
	}
	...
	void FinishDrag(int32_t tileX, int32_t tileZ) {
		...
		if (mOnValidate && !mOnValidate(mStartTileX, mStartTileZ, tileX, tileZ, errorMsg)) {
			LOG_INFO("{} validation failed: {}", mToolName.c_str(), errorMsg.c_str());
	...
	void CancelDrag() {
		...
		ClearDragFeedback();
		LOG_TRACE("{} drag cancelled", mToolName.c_str());
	...
	bool ScreenToTileCoordinates(int32_t screenX, int32_t screenY, uint32_t& tileX, uint32_t& tileZ) const {
		if (!mpView3DWin || !mpTerrain) {
			LOG_ERROR("ScreenToTileCoordinates could not execute because View3DWin or Terrain is null");
			return false;
		}

		float worldCoords[3] = { 0.0f, 0.0f, 0.0f };
		const bool terrainQueryState = mpView3DWin->GetTerrainQueryEnabled();
		...
		if (pickResult) {
			...
			tileX = std::max(static_cast<uint32_t>(0), std::min(tileX, maxX - 1));
			tileZ = std::max(static_cast<uint32_t>(0), std::min(tileZ, maxZ - 1));
			return true;
		}
		return false;
	}
protected:
	...
	bool MarkSelected(uint32_t startX, uint32_t startZ, uint32_t endX, uint32_t endZ, cISTETerrain::eHilightColorType hilightColor = cISTETerrain::eHilightColorType::Red, bool clearOthers = true) {
		const SC4Rect rect = { std::min(startX, endX), std::min(startZ, endZ), std::max(startX, endX), std::max(startZ, endZ) };
		return this->MarkSelected(rect, hilightColor, clearOthers);
	}
	...
	bool MarkSelected(SC4Rect<uint32_t> rect, cISTETerrain::eHilightColorType hilightColor = cISTETerrain::eHilightColorType::Red, bool clearOthers = true) {
		const SC4Rect<int> signedRect = {
			static_cast<int>(rect.topLeftX),
			static_cast<int>(rect.topLeftY),
			static_cast<int>(rect.bottomRightX),
			static_cast<int>(rect.bottomRightY)
		};

		return this->MarkSelected(signedRect, hilightColor, clearOthers);
	}

	bool MarkSelected(SC4Rect<int> rect, cISTETerrain::eHilightColorType hilightColor = cISTETerrain::eHilightColorType::Red, bool clearOthers = true) {
		if (!mpTerrain || !mpTerrain->GetView()) {
			LOG_ERROR("MarkSelected: terrain or terrain view is null");
			return false;
		}

		if (rect.topLeftX < 0 || rect.topLeftY < 0 || rect.bottomRightX < 0 || rect.bottomRightY < 0 ||
			rect.topLeftX > mpTerrain->CellCountX() - 1 || rect.topLeftY > mpTerrain->CellCountZ() - 1 ||
			rect.bottomRightX > mpTerrain->CellCountX() - 1 || rect.bottomRightY > mpTerrain->CellCountZ()) {
			LOG_ERROR("MarkSelected: invalid coordinates, out of bounds");
			return false;
		}
