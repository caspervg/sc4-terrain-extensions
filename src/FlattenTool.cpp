	...
	void Execute(const args::ArgumentParser& parser) override {
		...
		if (isRectangleMode) {
			mLogger->WriteLineFormatted(LogLevel::Info,
				"Flattening rectangle from (%d,%d) to (%d,%d)", tileX1, tileZ1, tileX2, tileZ2);
		} else {
			mLogger->WriteLineFormatted(LogLevel::Info,
				"Flattening single tile (%d,%d)", tileX1, tileZ1);
		}
...
private:
	void FlattenArea(int x1, int z1, int x2, int z2, float specifiedHeight, const std::string& mode, bool isRectangle, bool hasHeight) {
		...
		mLogger->WriteLineFormatted(LogLevel::Info, "Target height: %.2f", targetHeight);
		...
		mLogger->WriteLine(LogLevel::Info, "Terrain flattening completed");
	}
	...
	float CalculateTargetHeight(int minX, int minZ, int maxX, int maxZ, float specifiedHeight, const std::string& mode, bool isRectangle, bool hasHeight) {
		...
		if (!isRectangle) {
			mLogger->WriteLine(LogLevel::Error, "Height parameter required for single tile mode");
